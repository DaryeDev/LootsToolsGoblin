    ...
    if ("registerDiscordServer" in request){
        ...
        browser.storage.sync.get(["token"]).then(function (resulto) {
            ...
            if (!token){
                ...
                fetch("https://dysontoolsserver.fly.dev/finishSetupDiscordServer?syncID="+request.registerDiscordServer+"&slt="+Base64.encode(token), {
                    method: 'GET',
                    headers: {},
                    mode: "no-cors"
                })
        ...
        browser.storage.sync.get(["token"]).then(function (result) {
            token = result.token
            if (!token){
                sendResponse("noLogin")
            } else {
